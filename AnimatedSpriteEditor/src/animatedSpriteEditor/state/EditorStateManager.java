...
public class EditorStateManager 
{
    ...
    // WE NEED TO KEEP CHECKING IF THERE IS ANY SHAPE
    // BEING COPIED OR CUT IN THE CLIP BOARD
    // @SIXIN
    ...
    /**
     * This method checks if the selected shape is cut.
     * 
     * @return true if the selected shape is cut.
     */
    public boolean isCut() { return isCut; }
    
    /**
     * This method checks if the selected shape is copied
     * 
     * @return true is the selected shape is copied.
     */
    ...
    public void processMousePress(int x, int y)
    {
        ...
        {
        	for(int i=pose.getShapesList().size()-1; i>=0; i--){
        		PoseurShape testShape = pose.getShapesList().get(i);
        		if (testShape.getShapeType()==PoseurShapeType.LINE 
        				&& ((PoseurLine)testShape).distant(poseSpaceX, poseSpaceY) < 5){
     ...
     * will cut the selected item and put it on the clip board.
     */
    public void cutSelectedItem()
    {
        ...
        //REMOVE THE ORIGINAL SHAPE FROM THE SHAPE LIST 
    ...
    /**
     * This method puts the item in the end of the pose list,
     * which is the same as moving it to the front.
     */
    public void moveToFrontSelectedItem()
    {
    ...
    /**
     * This method puts the item in the front of the pose list,
     * which is the same as moving it to the back.
     */
    public void moveToBackSelectedItem()
    {
        PoseurShape shapeToMove = selectedShape.clone();
        pose.getShapesList().remove(selectedShape);
    ...
    /**
     * This method will assign the selected thickness to the selected
     * shape, if there is one. 
     * 
     * @param selectedThickness The thickness used to change the selected
     * shape and the lineStrokeSelectionComboBox. 
     */
    public void selectedOutlineThickness(BasicStroke selectedThickness){
    	AnimatedSpriteEditor singleton = AnimatedSpriteEditor.getPoseur();
    	AnimatedSpriteEditorGUI gui = singleton.getGUI();
        ...
        if (isShapeSelected())
        {
        	selectedShape.setOutlineThickness(selectedThickness);
        	repaintCanvases();
        }
    }
    
    /**
     * This method will assign the selected transparency value
     * to the selected shape, if there is one.
     * 
     * @param selectedAlphaValue The transparency value used to 
     * change the selected shape and the alpha slider. 
     */
    public void selectedTransparency(int selectedAlphaValue){
    	AnimatedSpriteEditor singleton = AnimatedSpriteEditor.getPoseur();
    	AnimatedSpriteEditorGUI gui = singleton.getGUI();
        ...
        if (isShapeSelected())
        {
        	selectedShape.setAlpha(selectedAlphaValue);
        	repaintCanvases();
        }
    }
    ...
    private void repaintCanvases()
    {
        ...
        EditorCanvas trueCanvas = trueCanvasState.getCanvas();
        trueCanvas.repaint();
        EditorCanvas zoomableCanvas = zoomableCanvasState.getCanvas();
